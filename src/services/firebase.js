...
export const fetchPlants = async () => {
  try {
    ...
    // Track plants with missing inventory for logging purposes
    const plantsWithMissingInventory = [];
    ...
    const plantsArray = Object.keys(plantsData).map(key => {
      ...
      // Verify if inventory exists for this plant
      if (!inventoryData[plantId]) {
        plantsWithMissingInventory.push({
          id: plantId,
          name: plant.name || 'Unknown Plant'
        });
      }
      
      // If we have inventory data but the status is missing or invalid, log it
      if (inventoryData[plantId] && (!inventoryData[plantId].status || inventoryData[plantId].status === "Unknown")) {
        inventoryMismatches.push({
          id: plantId,
          name: plant.name || 'Unknown Plant',
          currentStock: inventoryData[plantId]?.currentStock
        });
      }
      
      // Get inventory or create a default with appropriate status based on stock
      const inventory = inventoryData[plantId] || createDefaultInventory(0);
      
      // If inventory exists but status is "Unknown", fix it based on stock level
      if (inventory.status === "Unknown" || !inventory.status) {
        inventory.status = getDefaultStatusFromStock(inventory.currentStock);
      }
    ...
    // Log any plants with missing inventory for troubleshooting
    if (plantsWithMissingInventory.length > 0) {
      console.warn(`WARNING: ${plantsWithMissingInventory.length} plants are missing inventory data:`, 
        plantsWithMissingInventory.map(p => `${p.name} (ID: ${p.id})`).join(', '));
    }
    
    // Log inventory mismatches
    if (inventoryMismatches.length > 0) {
      console.warn(`WARNING: ${inventoryMismatches.length} plants have invalid inventory status:`, 
        inventoryMismatches.map(p => `${p.name} (ID: ${p.id}, Stock: ${p.currentStock})`).join(', '));
    }
  ...
  try {
    ...
    const repairPromises = Object.keys(plantsData).map(async (key) => {
      ...
      if (!inventory.status || inventory.status === "Unknown") {
        ...
        const inventoryRef = ref(database, `inventory/${plantId}`);
        const fixedStatus = getDefaultStatusFromStock(inventory.currentStock);
...
export const updatePlant = async (plantId, plantData) => {
  try {
    ...
    // If no inventory provided, ensure one exists with default values
    else {
      // Check if inventory exists
      const inventorySnapshot = await get(ref(database, `inventory/${plantId}`));
