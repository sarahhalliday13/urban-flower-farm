...
const AdminOrders = () => {
  ...
  return (
    <div className="admin-orders-wrapper" style={{
      width: '100%',
      maxWidth: '1200px',
      margin: '0 auto',
                ...
                {orders
                  ...
                  .map(order => (
                    <React.Fragment key={order.id}>
                      <tr 
                        className={activeOrder === order.id ? 'order-row active' : 'order-row'}
                        onClick={() => setActiveOrder(activeOrder === order.id ? null : order.id)}
                      >
                        <td className="order-cell" data-label="Order">
                          <span className="order-id">{order.id || 'Unknown'}</span>
                        </td>
                        <td className="date-cell" data-label="Date">
                          <span className="order-date">{order.date ? new Date(order.date).toLocaleDateString() : 'Unknown'}</span>
                        </td>
                        <td className="customer-cell" data-label="Customer">
                          ...
                          <span className="customer-email">{order.customer?.email || 'Not provided'}</span>
                        </td>
