...
function Shop() {
  ...
  // New function for mobile pagination (simplified, larger touch targets)
  const renderMobilePaginationControls = () => {
    if (totalPages <= 1) return null;
    ...
    return (
      <div className="mobile-pagination-controls">
        <a
          href={`/shop?page=${Math.max(1, currentPage - 1)}&sort=${sortOption}${searchTerm ? `&search=${searchTerm}` : ''}`}
          className={`mobile-pagination-link prev ${currentPage === 1 ? 'disabled' : ''}`}
        ...
        <a
          href={`/shop?page=${Math.min(totalPages, currentPage + 1)}&sort=${sortOption}${searchTerm ? `&search=${searchTerm}` : ''}`}
          className={`mobile-pagination-link next ${currentPage === totalPages ? 'disabled' : ''}`}
      ...
      <section className="featured-plants">
        ...
        {sortedPlants.length > 0 && (
          <div className="pagination-info">
            ...
            {width > 991 && sortOption !== 'all' && (
              <span> (filtered from {getStatusCounts.all} total)</span>
            )}
          </div>
        )}
        
        {/* Desktop pagination at bottom (hidden on mobile) */}
        <div className="desktop-pagination-bottom">
          {renderPaginationControls()}
        </div>
